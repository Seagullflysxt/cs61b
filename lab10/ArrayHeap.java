...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if (contents[pIndex].priority() > contents[index].priority()) {
            swap(pIndex, index);
            swim(pIndex);
        }
        //return;
    ...
    private void sink(int index) {
        ...
        int minLinkChildren = min(lIndex, rIndex);
        if (contents[index].priority() > contents[minLinkChildren].priority()) {
            swap(index, minLinkChildren);
            sink(minLinkChildren);
        }
        //return;
    }
    ...
    public void changePriority(T item, double priority) {
        ...
        for (int i = 0; i < size(); i += 1) {
            if (contents[i].item().equals(item)) {
                double oldP = contents[i].priority();
